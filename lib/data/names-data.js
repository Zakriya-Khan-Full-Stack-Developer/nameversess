...
export async function readNameData(religion, slug) {
  ...
  try {
    ...
    // Runtime on Netlify should fetch the JSON from the public asset URL instead of bundling the full
    // public/names dataset into the serverless function. This prevents the Netlify function from
    // exceeding the 250 MB upload limit while still serving individual name records on demand.
  }

  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'https://nameverse.site';
  const assetUrl = new URL(`/names/${normalizedReligion}/${normalizedSlug}.json`, siteUrl).toString();
